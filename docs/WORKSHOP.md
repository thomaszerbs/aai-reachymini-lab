...
> To edit a file, open it in the editor on screen (or `nano emo_v1.py`).
...
Stop it with **Ctrl+C**, open **`emo_v1.py`**, and find the big

```text
# >>> TRY ME <<<  Mini-lab Station 1
```

block near the top. Change one thing, save, and re-run `python emo_v1.py --chat`:
...
Ctrl+C, open **`emo_v6.py`**, find the `# >>> TRY ME <<< Mini-lab Station 2` block:
...
Re-run `python emo_v6.py --chat` and hear the difference.
...
1. Open **`emo_v8.py`**, find the `# >>> TRY ME <<< Mini-lab Station 3` block, and
...
2. **The fun part:** ask the staff to unplug the network cable / turn off Wi-Fi,
   then run `python emo_v8.py --chat` again. **Reachy keeps thinking and talking** —
...
Ctrl+C, open **`emo_v9_vision.py`**, find `# >>> TRY ME <<< Mini-lab Station 4`,
...
|--------|---------|---------------|
| 1 | `python emo_v1.py --chat` | trigger words / `NOD_AMPLITUDE_DEG` / persona |
| 2 | `python emo_v6.py --chat` | `DEFAULT_VOICE` / persona |
| 3 | `python emo_v8.py --chat` | persona / `DEFAULT_PIPER_MODEL` |
| 4 | `python emo_v9_vision.py` | `VISION_PROMPT` |
...
| Station 4: "Could not read camera" | Ask staff — the camera device may need selecting: `python emo_v9_vision.py --camera-device /dev/video2`. |
