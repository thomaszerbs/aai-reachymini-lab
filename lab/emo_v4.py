#!/usr/bin/env python3
"""emo_v4.py - Reachy Mini "sees" with a local vision model (Offline)
...
same offline Piper-TTS voice + emotion motions used in Station 3 (emo_v3).
...
Usage:
  python lab/emo_v4.py                        # push-to-look: press Enter, Reachy describes the scene
  python lab/emo_v4.py --gentle               # subtler motions for nearby humans
  python lab/emo_v4.py --camera-device /dev/video2
  python lab/emo_v4.py --save-frame look.jpg  # also save what the camera saw (debugging)
"""
...
# so the script works when run as `python lab/emo_v4.py` from the repo root.
...
# Change what Reachy looks for, then re-run `python lab/emo_v4.py`.
