#!/usr/bin/env python3
"""
emo_v3.py - Enhanced emotional controller for Reachy Mini with Ollama
WITH PARALLEL ACTIONS - Actions happen during text streaming

Key improvement from emo_v2:
- Actions triggered in parallel with text output
...
class ParallelChatAppV3:
    """Chat application with parallel action execution during text streaming"""
    ...
    def start_chat(self):
        ...
        print("=" * 60)
        print("Features:")
        print("1. Parallel actions during text streaming")
        print("2. Uses recorded moves library for richer expressions")
        print("3. Enhanced emotion detection with emoji support")
        print("4. Intensity-based action selection")
        print("5. Fallback to simple actions")
        print("=" * 60)
    ...
    def test_all_moves(self):
        ...
        try:
            with ReachyMini(media_backend="no_media") as reachy:
                controller = ParallelEmotionController(reachy, debug=self.debug)
...
def main():
    ...
    app = ParallelChatAppV3(model=args.model, ollama_url=args.url, debug=args.debug)
